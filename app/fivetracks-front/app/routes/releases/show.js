...
export default Route.extend({
  model(params){
    return this.get('store').findRecord('release', params.release_id);
  },
  actions: {
    refreshPage(){
      var model = this.modelFor('releases.show').hasMany('releaseTracks')
    ...
    allQueueSave(release){
      let r = this;
      let model = this.modelFor('releases.show')
      console.log(model)
